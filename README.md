...
# 5/22
## 자바의 GUI
- AWT와 Swing 패키지 두가지가 있음, 보통Swing 사용
## 컬렉션 개념
- 요소(element) 라고 불리는 가변 개수의 객체들의 저장소
- 고정 크기의 배열을 다루는 어려움을 해소
### 제네릭 기법
- 특정 타입만 다루지 않고, 여러 종류의 타입으로 변신할 수 있도록 클래슨 메서드를 일반화시키는 기법
- 요소는 객체만 가능
```java
Vector(int) v = new Vector(int)(); //int 불가
...
### HashMap<K,V>
- 키와 값으로 구성되는 요소를 다루는 컬렉션
- K : 키로 사용할 요소의 타입
- V : 값으로 사용할 요소의 타입
## MATH 클래스
- 기본 산술 연산 메서드를 제공함
- 모든 메서드는 static로 선언 (클래스 이름으로 호출 가능)
```java
for(int x=0; x<10; x++){
    int n = (int)Math.random()*100+1; //1 ~ 100까지 정수 생성
}
```
## StringBuffer 클래스
- 가변 스트링을 다루는 클래스
